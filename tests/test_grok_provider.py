...
class GrokProviderTests(unittest.TestCase):
    ...
    def test_generate_returns_error_when_api_key_is_missing(self):
        with mock.patch.dict(
            ...
            result, error = grok_provider.generate(
    ...
    def test_generate_accumulates_streamed_response_and_closes_stream(self):
        ...
        with mock.patch.object(
            ...
            {
                "XAI_API_KEY": "test-key",
                "CODA_GROK_MODEL": "test-model",
            },
        ):
            result, error = grok_provider.generate(messages)
        ...
        openai_module.OpenAI.assert_called_once_with(
            api_key="test-key",
            base_url=grok_provider.GROK_BASE_URL,
        )
